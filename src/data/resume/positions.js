const positions = [
  {
    company: 'Major League Hacking',
    position: 'Coach/Streamer',
    link: 'https://mlh.io/',
    daterange: 'April 2023 - Present',
    points: [
      'Streamed project-based interactive coding sessions to thousands of people via Twitch and YouTube',
    ],
  },
  {
    company: 'Dell Technologies',
    position: 'Software Engineering Intern',
    link: 'https://dell.com/en-us',
    daterange: 'May 2023 - August 2023',
    points: [
      'Collected Docker and Kubernetes container logs via Fluentd and processed them using Python',
      'Used Kibana and Elasticsearch (EFK stack) to present log data in a human-intelligible format',
    ],
  },
  {
    company: 'DiscoverSTEM',
    position: 'Innovation Facilitator',
    link: 'https://discoverstem.info',
    daterange: 'January 2021 - December 2022',
    points: [
      'Facilitated innovation teams to identify and solve real-world problems',
      'Trained students to set up non-invasive neuroimaging system hardware (assembling electrodes, gel usage, Bluetooth optimization) and software (Unicorn BCI Suite, MATLAB) for recording electroencephalography (EEG) data',
      'Lead a team in constructing a thought-controlled wheelchair prototype',
      'Conducted research on steady state visually evoked potentials (SSVEP), the P300 paradigm, and motor imagery to identify the best EEG based BCI for relevant application',
      'Research presented at Advancing Healthcare Innovation Summit (AHIS) 2022 and published in Innovation Journals (DOI 10.36401/IDDB-22-X2)',
    ],
  },
  {
    company: 'Raytheon Intelligence & Space',
    position: 'Software Engineering & Machine Learning Intern',
    link: 'https://www.raytheon.com/',
    daterange: 'May 2022 - August 2022',
    points: [
      'Developed a machine learning software to predict hardware faults',
      'Refactored PERL scripts to process failure log files 250% faster',
      'Presented project to senior leadership, Air Force, and Lockheed Martin',
    ],
  },
];

export default positions;
